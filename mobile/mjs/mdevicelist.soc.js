  ...
  if (on == 1) {
    ...
    if (v == 1) {
      if (s >= 120) {
        ...
        var acc =
          typeof st != undefined && st != null && st.indexOf("3005") >= 0;
        if (acc) {
          ret.sta = 30;
        } else {
          ret.sta = 1;
    ...
    if (t == null || t == "undefined" || t == "") {
      ...
      var time =
        newDate(t).getTime() + new Date().getTimezoneOffset() * 60 * 1000;
      ...
      if (s == -1) {
        ...
        ret.tip =
          ">" + parseInt(timeout / (3600 * 24 * 7)) + WP.JS_TIMEOUT_WEEK;
      } else if (timeout >= 3600 * 24 * 30 && timeout < 3600 * 24 * 365) {
        ret.tip =
          ">" + parseInt(timeout / (3600 * 24 * 30)) + WP.JS_TIMEOUT_MON;
      } else {
        ret.tip =
          ">" + parseInt(timeout / (3600 * 24 * 365)) + WP.JS_TIMEOUT_YEAR;
      }
    }
  }
  return ret;
...
function getSpeedColor(s) {
  //color indication to show speed levels
  if (s >= 120) {
    return "#FF002A";
  } else if (s >= 90) {
    return "#9A009C";
...
function addGroupItem(
  ...
  t =
    t == null || t.length == 0
      ? t
      : $.format.date(t, JS_DEFAULT_DATETIME_fmt_JS);
  ts =
    ts == null || ts.length == 0
      ? ts
      : $.format.date(ts, JS_DEFAULT_DATETIME_fmt_JS);
  ...
  if (!$KI) {
    ...
    function appendCardToElement(targetElement) {
      var cardHtml = `
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="M9 6.75V15m6-6v8.25m.503 3.498l4.875-2.437c.381-.19.622-.58.622-1.006V4.82c0-.836-.88-1.38-1.628-1.006l-3.869 1.934c-.317.159-.69.159-1.006 0L9.503 3.252a1.125 1.125 0 00-1.006 0L3.622 5.689C3.24 5.88 3 6.27 3 6.695V19.18c0 .836.88 1.38 1.628 1.006l3.869-1.934c.317-.159.69-.159 1.006 0l4.994 2.497c.317.158.69.158 1.006 0z" />
      ...
      // Append the HTML structure to the targetElement
      $(targetElement).append(cardHtml);
    }

	appendCardToElement('#testElement');

    //end state
    $tr = $("<tr class='end_state'></tr>").insertAfter($KI);
    $("<td id='testElement'></td>").text("").appendTo($tr);
    ...
    $(
      "<li style='width: " + stateLength * 0.2 + "px;' id='temp_1'></li>"
    ).appendTo($ul);
    $(
      "<li style='width: " + stateLength * 0.2 + "px;' id='fuel_1'></li>"
    ).appendTo($ul);
    $(
      "<li style='width: " +
        stateLength * 0.2 +
    ...
    $(
      "<li style='width: " + stateLength * 0.2 + "px;' id='max_speed_24'></li>"
    ).appendTo($ul);
    $(
      "<li style='width: " +
        stateLength * 0.197 +
        "px;' id='moving_time_24'></li>"
    ).appendTo($ul);
    $(
      "<li style='width: " + stateLength * 0.2 + "px;' id='idle_time_24'></li>"
    ).appendTo($ul);
    $(
      "<li style='width: " + stateLength * 0.2 + "px;' id='stop_time_24'></li>"
    ).appendTo($ul);
    $(
      "<li style='width: " +
        stateLength * 0.2 +
        "px;' id='engine_time_24'></li>"
    ).appendTo($ul);
    $(
      "<li style='width: " + stateLength * 0.197 + "px;' id='total_mil'></li>"
    ).appendTo($ul);
    $(
      "<li style='width: " + stateLength * 0.205 + "px;' id='door_state'></li>"
    ).appendTo($ul);
    ...
    if ($KI.attr("g") != gid) {
      ...
      $(
        "<li style='width: " +
          stateLength * 0.2 +
          "px;' id='idle_time_24'></li>"
      ).appendTo($td);
      $(
        "<li style='width: " +
          stateLength * 0.2 +
          "px;' id='stop_time_24'></li>"
      ).appendTo($td);
      $(
        "<li style='width: " +
          stateLength * 0.2 +
  ...
  if (map) {
    try {
      ...
      if ($KI.find("td:eq(1) input").is(":checked") && historyid != n) {
        ...
        $KI.find("td:eq(0) input").is(":checked") &&
        selid == n &&
        !historymode
      ) {
    ...
    function (data) {
      try {
        if (typeof toastShow == "undefined" || toastShow == false) {
          ...
          for (var i = 0; i < json.length; i++) {
            var jo = json[i];
            var $tbody = $("<tbody class='event-table-body'></tbody>").appendTo(
              $("#event-table")
            );
          ...
          for (var i = 0; i < json.length; i++) {
            ...
            if (jo.n > lastAlarmId) {
              ...
              toastr.error(
                $.format.date(jo.t, JS_DEFAULT_DATETIME_fmt_JS) + "<br>" + jo.a,
                toastShow.c,
                {
                  closeButton: true,
                  debug: false,
...
>>>>>>> bf8c9422f3fdacbfcd9dd3c3398d0c8badd7389a
}

function clearAlarm(keyid) {
  var item = $("#tab_all .tree_table")
    .find("tbody tr[n=" + keyid + "]")
    .find("td:eq(0) input");
  if (!item.is(":checked")) {
    ...
    $.post(
      ...
      function (data) {
        ...
        if (
          ...
          typeof jo != "undefined" &&
          jo.jsons != null &&
          typeof jo.jsons != "undefined" &&
          jo.jsone != null &&
          typeof jo.jsone != "undefined"
        ) {
          ...
          } else if (jo.jsons[0].at == 4) {
            //marker
            var apts = jo.jsons[0].ap.split(",");
            startLocation = [apts[0], apts[1]];
          } else if (jo.jsons[0].at == 5) {
            //Polyline
            var route = [];
            var points = jo.jsons[0].ap.split(";");
          ...
          } else if (jo.jsone[0].at == 4) {
            //marker
            var apts = jo.jsone[0].ap.split(",");
            endLocation = [apts[0], apts[1]];
          } else if (jo.jsone[0].at == 5) {
            //Polyline
            var route = [];
            var points = jo.jsone[0].ap.split(";");
...
function showSharePosition(keyid) {
  ...
  $dlg.css(
    "top",
    Math.round(document.body.clientHeight / 2 - ($dlg.height() / 3) * 2) + "px"
  );
  $dlg.css(
    "left",
    Math.round(document.body.clientWidth / 2 - $dlg.width() / 2) + "px"
    ...
    if (protocolCmd[i].pid == protocolid && protocolCmd[i].cid == commandid) {
      ...
      /*DEF_VALUE == %DATE%*/
        var selector =
          ...
          "_" +
          protocolCmd[i].psn;
        $input = $("<input class='" + selector.substring(1) + "'></input>")
          .addClass("itime")
          .addClass("enablebox")
          .appendTo($tvalue);
        $tvalue.attr("ptype", 1);
...
function dlgDeviceInfo(keyid, ios) {
  ...
  $.get(
    "../stastics.ajax.php",
    { rtime: true, objid: keyid, type: 51, etime: getCurentDateTime() },
    function (data) {
      ...
      for (var i = 0; i < rows.length; i++) {
        if (rows[i] != null && rows[i].length > 0) {
          ...
          if (row != null && row.length == 4) {
            ...
            //pressure
            datas.tire.push(
              tpmsUnitConversion(parseFloat(row[1]), JS_UNIT_TPMS)
            );
            /*if(JS_UNIT_TPMS == 1){
            ...
            datas.temp.push(
              tempUnitConversion(parseFloat(row[2]), JS_UNIT_TEMPERATURE)
            );
      ...
      $("#cmdparam")
        ...
        .each(function () {
          ...
          if (
            ...
            if (
              parseFloat(cinput) < limit_min ||
              parseFloat(cinput) > limit_max
            ) {
              $(this).css("background-color", "red");
              $("#cmderror")
                .css("display", "block")
      ...
      $("#cmdparam tbody")
        ...
        .each(function () {
          ...
          if (param.attr("ptype") == 0) {
            params =
              params + $(this).children().eq(1).find("select").val() + ",";
          } else {
            params =
              params + $(this).children().eq(1).find("input").val() + ",";
          }
        });
...
function updateOne(tr) {
  ...
  var tempstr = (temp_1 + "," + temp_2 + UNIT_TEMP)
    .replace("--,", "")
    .replace(",--", "");
  ...
  var fuelstr = (fuel_1 + "," + fuel_2 + UNIT_FUEL)
    .replace("--,", "")
    .replace(",--", "");
  ...
  try {
    $.get("../playback.ajax.php", param, function (data) {
      ...
      try {
        ...
        /*alert(e.message)*/ error(showLoading(false));
      }
    });
  } catch (e) {
    /*alert(e.message)*/ error(showLoading(false));
...
function updateShow() {
  ...
  if (typeof alarms != "undefined" && alarms > 0) {
    ...
    $("#alarm_msg").html(
      "<a href='#'>" + JS_TIP_OBJ_ALARM + "(" + alarms + ")</a>"
    );
...
function updateDevice(json, current, map, first) {
  var needupdate =
    json != null && typeof json != "undefined" && json.length > 0;
...
function getExpireObjByDays() {
  ...
  $(r).each(function (i, value) {
    if ($(value).find("td:eq(2)").attr("ex") != null) {
      var ex =
        (newDate($(value).find("td:eq(2)").attr("ex")).getTime() -
          new Date().getTime()) /
        (24 * 3600 * 1000);
      if (ex <= 0) {
...
function getTopMileageEngine() {
  ...
  $(r).each(function (i, value) {
    ...
    if (
      sval != "-1" &&
      zt != null &&
      new Date().getTime() - newDate(zt).getTime() < 24 * 3600 * 1000
    ) {
      allObjIos.push({
        c: c,
        m: m24h == null ? 0 : m24h,
        e: e24h == null ? 0 : parseFloat(e24h) / 3600,
        i: i24h == null ? 0 : parseFloat(i24h) / 3600,
        o: o24h == null ? 0 : parseFloat(o24h) / 3600,
      });
    }
  });
...
function createObjStateBar(
  ...
  (function (H) {
    H.wrap(H.Tooltip.prototype, "getLabel", function (proceed) {
      ...
      if (this.container) {
        H.css(this.container, { zIndex: 9999 });
      }
  ...
  var toolTip_exp = [ex_0, ex_1, ex_7, ex_15, ex_30];
  var sta_exp_time_chart = Highcharts.chart("sta_exp_time", {
    ...
    tooltip: {
      ...
      pointFormatter: function () {
        var string = "";
        Highcharts.each(
          toolTip_exp[this.series.data.indexOf(this)],
          function (p) {
  ...
  var toolTip_speed = [s_0_40, s_40_80, s_80_90, s_90_120, s_120];
  var sta_speed_range_chart = Highcharts.chart("sta_speed_range", {
    ...
    tooltip: {
      ...
      pointFormatter: function () {
        var string = "";
        Highcharts.each(
          toolTip_speed[this.series.data.indexOf(this)],
          function (p) {
            string += '<a href = "">' + p + "</a><br>";
          }
        );
