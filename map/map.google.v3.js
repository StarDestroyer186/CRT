function MapClassImpl(container, opts, searchbar, extbar, history) {
	...
	if(extbar == true || history == true){
		...
		$("<td width='50'></td>").text(120).appendTo($trspeed);
	...
	if(extbar == true){
		...
		$("<td align='center' height='28px' width='28px' bgcolor='#FFFFFF'><img id='ed_asset' class='tool_active' height='14px' width='14px' src='../img/tool_object-arrow.svg'/></td>").appendTo($trtool1);
		var $trtool2 = $("<tr></tr>").appendTo($tbody);
		$("<td align='center' height='28px' width='28px' bgcolor='#FFFFFF'><img id='ed_marker' class='tool_active' height='14px' width='14px' src='../img/tool_markers.svg'/></td>").appendTo($trtool2);
		var $trtool3 = $("<tr></tr>").appendTo($tbody);
		$("<td align='center' height='28px' width='28px' bgcolor='#FFFFFF'><img id='ed_zone' class='tool_active' height='14px' width='14px' src='../img/tool_zones.svg'/></td>").appendTo($trtool3);
		var $trtool4 = $("<tr></tr>").appendTo($tbody);
		$("<td align='center' height='28px' width='28px' bgcolor='#FFFFFF'><img id='ed_clusters' class='tool_active' height='14px' width='14px' src='../img/tool_clusters.svg'/></td>").appendTo($trtool4);
		
		if (!!window.ActiveXObject || "ActiveXObject" in window){
			//If IE can not use MeasureTool			
		}else{
			var $trtool5 = $("<tr></tr>").appendTo($tbody);
			$("<td align='center' height='28px' width='28px' bgcolor='#FFFFFF'><img id='ed_measure' style='opacity: 0.5' height='12px' width='12px' src='../img/tool_ruler.svg'/></td>").appendTo($trtool5);
			
			this.measureTool = new MeasureTool(this.wndMap, {
				contextMenu: false,
				showSegmentLength: true,
				tooltip: false,
				unit: MeasureTool.UnitTypeId.METRIC,
				totalDistance: JS_TOTAL_DISTANCE,
				totalArea: JS_TOTAL_AREA
			});
		}
	}
...
MapClassImpl.prototype = {
	...
	GeoNames: function(x, y, element, style) {
		...
		try {
			...
			$.ajax({
				url:'http://ditu.google.cn/maps/api/geocode/json?latlng='+y / 1000000.0+','+x / 1000000.0+'&sensor=false&language='+JS_CURRENT_LANG,
				type:'get',
				dataType:'json',
