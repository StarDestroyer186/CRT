function MapClassImpl(container, opts, searchbar, extbar, history, drivers4jb) {	
	...
	if(extbar == true){
		var $maptools = $('<div id="maptools" ontouchmove="event.preventDefault();" />').appendTo(container.parentElement);
		...
		$("<td align='center' height='28px' width='28px' bgcolor='#FFFFFF'><img id='ed_driver' style='opacity: 0.5' height='50%' width='50%' src='../img/steering_wheel_green.svg'/></td>").appendTo($trtool12);
		...
		$("<td align='center' height='28px' width='28px' bgcolor='#FFFFFF'><img id='ed_street_view' style='opacity: 0.5' height='50%' width='50%' src='../img/tool_streetview.svg'/></td>").appendTo($trtool10);
		var $trtool11 = $("<tr></tr>").appendTo($tbody);
		$("<td align='center' height='28px' width='28px' bgcolor='#FFFFFF'><img id='ed_task' style='opacity: 0.5' height='50%' width='50%' src='../img/tasknew.svg'/></td>").appendTo($trtool11);
		

	
		var $maptools_ext = $('<div id="maptools_ext" ontouchmove="event.preventDefault();" />').appendTo(container.parentElement);
		var $tbody_ext = $("<table></table>").appendTo($maptools_ext);
		var $tr_ext = $("<tr></tr>").appendTo($tbody_ext);
		$("<td align='center' height='28px' width='28px' bgcolor='#FFFFFF'><img id='ed_arrow' class='tool_active' height='50%' width='50%' src='../img/tool_arrow.svg'/></td>").appendTo($tr_ext);
		$("<td align='center' height='28px' width='28px' bgcolor='#FFFFFF'><img id='ed_point' style='opacity: 0.5' height='50%' width='50%' src='../img/tool_point.svg'/></td>").appendTo($tr_ext);
		$("<td align='center' height='28px' width='28px' bgcolor='#FFFFFF'><img id='ed_stop' class='tool_active' height='50%' width='50%' src='../img/tool_stop.svg'/></td>").appendTo($tr_ext);
		$("<td align='center' height='28px' width='28px' bgcolor='#FFFFFF'><img id='ed_event' class='tool_active' height='50%' width='50%' src='../img/tool_event.svg'/></td>").appendTo($tr_ext);		
		$("<td align='center' height='28px' width='28px' bgcolor='#FFFFFF'><img id='ed_route' class='tool_active' height='50%' width='50%' src='../img/tool_route.svg'/></td>").appendTo($tr_ext);		
		$("<td align='center' height='28px' width='28px' bgcolor='#FFFFFF'><img id='ed_snap' style='opacity: 0.5' height='50%' width='50%' src='../img/tool_snap.svg'/></td>").appendTo($tr_ext);								
	}
	
	var mylocation = $("<div bgcolor='#FFFFFF' id='mylocation' style='opacity: 0.5;' ontouchmove='event.preventDefault();'></div>").appendTo(container);
	
	var $playspeed =  $("<select bgcolor='#FFFFFF' id='playspeed' class='iselect' ontouchmove='event.preventDefault();'></select>").appendTo(container);
	var $item = $("<option></option>").appendTo($playspeed);
	$item.attr("value", 1000);
	$item.text('x1');
	
	$item = $("<option></option>").appendTo($playspeed);
	$item.attr("value", 500);
	$item.text('x2');
	...
	this.firstTime = 0;
	this.markerCluster.on('clusterclick', function (evt) {
		var secondTime = Date.parse(new Date());

		if (secondTime - self.firstTime < 1500) {
			self.firstTime = 0;
			evt.layer.zoomToBounds();
		} else {
			self.firstTime = secondTime;
			...
			clusterMarkers.forEach(function(marker) {			
				...
				$tr = $('<tr style="height: 25px;"></tr>').appendTo($tbody);
			...
			var popup = L.popup({offset: new L.Point(0, -10), minWidth : 250, maxHeight: 200})
	...
	map.on('popupopen', function(e) {
	   ...
	   if(marker == null || marker.properties == null || marker.properties.s == null){
		   return;
	   }
	   var speedmeterId = "sta_speedometer_" + marker.properties.n;
	   createSpeedometer(speedmeterId, marker.properties.s);
	   	 
	   if(typeof marker.properties == "undefined" || typeof marker.properties.ngeo == "undefined" || marker.properties.ngeo == false ||(marker.properties.anim != null && marker.properties.anim)){
		   return;
	   }
	  
	   var popup = marker.getPopup();
	   var content = popup.getContent();
	   var address = $(".infowindow .infobase ul .infoaddress"); 	 
	  
	   if($(content).find(".infobase ul .infoaddress").length > 0){
		   console.log("parse location");
		   self.GeoNames(marker.properties.x, marker.properties.y, address, "text", 0);		   
	   }
	...
	map.addEventListener('overlayadd', function(){
		$(".leaflet-control-layers").removeClass("leaflet-control-layers-expanded");
	});
	map.addEventListener('overlayremove', function(){
		$(".leaflet-control-layers").removeClass("leaflet-control-layers-expanded");
	});
...
MapClassImpl.prototype = {
	...
	ActiveMeasureTool: function(active){
		if(active){
			...
			this.wndMap.on('click', this.areaMeasure.click).on('dblclick', this.areaMeasure.dblclick);
		}else{
	...
	GeoNames: function(x, y, element, style, geocoders, isAsync) {
		try {
			$.ajaxSettings.async = (isAsync == null || isAsync) ? true:false;
			var self = this;
	...
	NewMarker: function(keyid, title, nc, si, x, y, ico, sta, dir, tips, view, extbar, speed, time, stime, st, io, dt, jb, dn) {
		...
		.bindPopup(tips, {offset: new L.Point(0, -10), minWidth : 320});/*.openPopup();
	...
	UpdateMarker: function(marker, title, nc, si, x, y, ico, sta, dir, tips, view, speed, time, stime, st, io, dt, jb, dn, du) {							
		var WP = window.parent;
		/*if(typeof view != "undefined" && view == true){
			if (marker.properties.x != x || marker.properties.y != y || marker.properties.ts != stime){			
				this.ClosePopup();
			}			
		}*/
		var mt = 0; //0: offline, 1: online and static, 2: online and moving, 7: online and alarm, 30: online and idle, 50: other	
		marker.properties.ngeo = false;
		...
		if (marker.properties.x != x || marker.properties.y != y) {
			...
			if (this.wndMap.getBounds().contains(pt) && this.wndMap.getBounds().contains(pts) && m && "_popup" in m) {			
				this.animationMarker[title] = marker;
			    var isPopup = marker.isPopupOpen();
				marker.properties.anim = true;
				marker.closePopup();
				...
				marker.setEnd(function(){
					...
					marker.properties.ngeo = true;
					marker.properties.anim = false;
					that.MoveTop(marker);
	...
	LocateMarker: function(marker, center, zoomIn, bound, popup) {
		...
		if(marker.isPopupOpen() || (typeof zoomIn != "undefined" && zoomIn == true) || (typeof popup != "undefined" && popup == true)){
			if(marker.properties != null && marker.properties.ngeo != null && typeof marker.properties.ngeo != "undefined"){
				marker.properties.ngeo = true;
			}			
			marker.closePopup();
			marker.openPopup();				
	...
	DisplayMyLocation: function(position,needCenter){
		if(!this.myLocationMarker){
			...
			this.myLocationMarker = L.marker([position.lat, position.lng], {icon: icon})
			.bindPopup(position.lat+','+position.lng, {offset: new L.Point(0, -10)}).openPopup()
			...
			this.myLocationMarker.setLatLng([position.lat, position.lng]);
		}
		
		if(needCenter){
			this.wndMap.panTo(this.myLocationMarker.getLatLng());	
		}		
	},
