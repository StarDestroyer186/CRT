function MapOperat(container, opts, searchbar, extbar, history) {
	this.markers = [];
	this.zoneMarkers = [];
	this.drivers = [];
	this.drivers4jb = [];
	...
	this.map_obj = new MapClassImpl(container_div, opts, searchbar, extbar, history, this.drivers4jb);
...
MapOperat.prototype = {
	ActiveNavigationTool: function(active){
		this.map_obj.ActiveNavigationTool(active);
	},
	...
	GeoNames: function(x, y, element, style, geocoders) {
		try {
			this.map_obj.GeoNames(x, y, element, style, geocoders)
		} catch(e) {}
	},
	...
	DrawIcon: function(keyid, title, nc, si, v, x, y, ico, sta, dir, time, stime, spd, alarm, view, s, st, io, dt, jb, dn, du) {
		...
		//try {
			...
			if (typeof this.map_obj != "undefined" && x != null && x != 0 && y != null && y != 0) {
				...
				/*driver info*/				
				if(this.extbar == true || this.history == true){
					tips += "<div class='infodriver'>" +
							"<ul>" +
							"<li id='idrvdef_"+keyid+"'>" +
							idrvdef +
							"</li>"+
							"<li id='ndrvdef_"+keyid+"'>" +
							ndrvdef+
							"</li>"+							
							"</ul>"+
							"</div>";
				...
				if(s > 0 || moving_time != null){
					tips += "<li title='"+WP.JS_MOVING+"' class='infospeed' style='padding-left: 18px; background: url(../img/move.svg) no-repeat 0px; background-size : 18px 18px;'>" + " <span>" + "&nbsp;&nbsp;" + moving_static_time + "</span></li>";
				}else{
					tips += "<li title='"+WP.JS_STATIC+" 'class='infospeed' style='padding-left: 18px; background: url(../img/parked.svg) no-repeat 0px; background-size : 19px 19px;'>" + " <span>" + "&nbsp;&nbsp;" + moving_static_time + "</span></li>";
				}
				
				tips += "<li title='"+WP.JS_GPS_TIME+" ' class='infoltime'>" + " <span>" + "&nbsp;&nbsp;" + time + "</span></li>" +
				"<li title='"+WP.JS_ODOMETER+" ' class='odometer'>" + " <span>" + "&nbsp;&nbsp;" + odometer + "</span></li>" +
				"<li>" + " <span title='"+WP.JS_GPS_SIGNAL+"' class='infogps'>" + gps + "</span>&nbsp;&nbsp;<span title='"+WP.JS_GSM_SIGNAL+"' class='infogsm'>" + gsm + "</span>&nbsp;&nbsp;<span title='"+WP.JS_PASSENGER+"' class='infopassenger'>" + passenger + "</span></li>" +
				//"<li>" + " <span class='infogsm'>" + "&nbsp;&nbsp;" + gsm + "</span></li>" +					
				"<li title='"+WP.JS_POSITION+"' class='infolocal'>" + " <span>" + "&nbsp;&nbsp;<a style ='color: #4D8ED9; text-decoration: none;' target='_blank' href="+JS_GOOGLE_MAP_BASE_LINK+"/maps?hl=en&q=" + (y / 1000000).toFixed(5) + "," + (x / 1000000).toFixed(5) + ">" +  (y / 1000000).toFixed(5) + " &#176;," + (x / 1000000).toFixed(5) + " &#176;</a></span></li>";
				
				/*click action*/
				if(this.extbar == true){
					tips += "<li><a href='#' class='button trackby' title='"+WP.JS_CMD_TRACKBY+"' onClick='menuselect(1,\""+keyid+"\");'></a>"+
						"<a href='#' class='button objinfo' title='"+WP.JS_CMD_OBJINFO+"' onClick='menuselect(2,\""+keyid+"\");'></a>"+
						"<a href='#' class='button sendcmd' title='"+WP.JS_CMD_SENDCMD+"' onClick='menuselect(3,\""+keyid+"\","+dt+");'></a>";
					/*camera*/
					tips += "<a href='#' class='button lastpic' title='"+WP.JS_LAST_PHOTO+"' onClick='menuselect(5,\""+keyid+"\");'></a>";
					tips += "<a href='#' class='button lastvoice' title='"+WP.JS_LAST_VOICE+"' onClick='menuselect(6,\""+keyid+"\");'></a>";
					tips += "<a href='#' class='button shareposition' title='"+WP.JS_SHARE_POSITION+"' onClick='menuselect(7,\""+keyid+"\");'></a>";
                    if(alarm>0){
                        tips += "<a href='#' class='button altifno' title='"+WP.JS_CMD_ALTINFO+"' onClick='menuselect(4,\""+keyid+"\");'></a>";
                    }
					tips += "</li>";
				}
				...
				if(this.extbar == true || this.history == true){
					...
					if(st != null && typeof st != "undefined" && st.length > 0){
						/*ACC state*/
						if(st.indexOf('3005') >= 0){
							tips +=	"<li title='"+WP.JS_ENGINE+"' class='infoacc' style='padding-left: 18px; background: url(../img/engine_on.svg) no-repeat 0px; background-size : 18px 18px;'>" + " <span>" + "&nbsp;" + acc_state + "</span></li>";
						}else if(st.indexOf('3006') >= 0){
							tips +=	"<li title='"+WP.JS_ENGINE+"' class='infoacc' style='padding-left: 18px; background: url(../img/engine_off.svg) no-repeat 0px; background-size : 18px 18px;'>" + " <span>" + "&nbsp;" + acc_state + "</span></li>";
						}else{
							tips += "<li title='"+WP.JS_ENGINE+"' class='infoacc'>" + " <span>" + "&nbsp;" + acc_state +  "</span></li>";
						}
					}else{
						tips += "<li title='"+WP.JS_ENGINE+"' class='infoacc'>" + " <span>" + "&nbsp;" + acc_state +  "</span></li>";
					}

					tips += "<li title='"+WP.JS_FUEL+"' class='infofuel'>" + " <span>" + "&nbsp;" + fuelstr + "</span></li>" +	
							"<li title='"+WP.JS_TEMP+"' class='infotemp'>" + " <span>" + "&nbsp;" + tempstr + "</span></li>" +
							"<li title='"+WP.JS_BAT+"' class='infovolt'>" + " <span>" + "&nbsp;" + battery + "</span></li>" +
							"<li title='"+WP.JS_WEIGHT+"' class='infoweight'>" + " <span>" + "&nbsp;" + weight + "</span></li>" +
							"</ul>"+
							"</div>";
				}
				tips += "</div>";								
	...
	DrawTrackLine: function(keyid, tracks, opts, stops, nstops, events, nevents, nangles, ntimes, moves, nsnap, du) {
		try {
			this.map_obj.DrawTrackLine(keyid, tracks, opts, stops, nstops, events, nevents, nangles, ntimes, moves, nsnap, du)
		} catch(e) {}
	},
	...
	LocateMarker: function(keyid, center, zoomIn, bound) {
		this.HideShowMarker(false, -1000);
